...
export default function CustomBtn(btnProps: { customClass?: string, text: string, linkHref: string }) {
    return (
        <Link href={btnProps.linkHref} className={clsx(
            "smooth py-4 px-12 active:rotate-3 hover:text-white cursor-pointer active:scale-90 bg-transparent text-[#6fad45] border-[#6fad45] w-fit select-none border-2 border-dashed hover:border-transparent relative overflow-hidden h-fit",
